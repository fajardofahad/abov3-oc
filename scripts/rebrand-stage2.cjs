#! /usr/bin/env node
/**
 * scripts/rebrand-stage2.cjs
 * CommonJS copy of rebrand-stage2.js to run under projects with "type": "module".
 * This file intentionally uses require() so it can be executed with `node` when
...
const batches = {
  1: [
    ...
    { from: '"name": "opencode', to: '"name": "opencode-legacy' },
  ],
  2: [
    // examples and imports in JS/TS
    { from: 'from "abov3/', to: 'from "abov3/' },
...
function main() {
  ...
  if (!batch) {
    console.error('Usage: rebrand-stage2.cjs --batch <n> [--dry-run] [--commit]')
    process.exit(2)
  }
