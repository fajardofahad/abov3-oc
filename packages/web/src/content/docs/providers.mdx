---
title: Providers
description: Using any LLM provider in abov3.
---

import config from "../../../config.mjs"
export const console = config.console

ABOV3 uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support for **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using `abov3 auth login`.
2. Configure the provider in your ABOV3 config.

---

### Credentials

When you add a provider's API keys with `abov3 auth login`, they are stored
in `~/.local/share/abov3/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your abov3
config.
...
abov3 zen is a list of models provided by the abov3 team that have been
tested and verified to work well with abov3. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with abov3 zen.
:::
...
It works like any other provider in abov3. And is completely optional to use
it.

---

## Directory
You can configure abov3 to use local models through LM Studio.
  "$schema": "https://abov3.ai/config.json",
...
You can configure abov3 to use local models through Ollama.
  "$schema": "https://abov3.ai/config.json",
To use Amazon Bedrock with abov3:
...
1. Head over to the **Model catalog** in the Amazon Bedrock console and request
   ...
   :::
1. Make sure you are on the latest version of abov3.
   $ abov3 auth login
1. **Check the auth setup**: Run `abov3 auth list` to see if the credentials
2. For custom providers, check the abov3 config and:
   - Make sure the provider ID used in `abov3 auth login` matches the ID in your abov3 config.
   ...
   Once you have one of the above, set it while running abov3.
1. Make sure you are on the latest version of abov3.
   ```bash
   AWS_ACCESS_KEY_ID=XXX abov3
   ```
...
Once you've singed up, run `abov3 auth login` and select Anthropic.
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for abov3 to work properly.
   :::

3. Run `abov3 auth login` and select **Azure**.
...
2. Run `abov3 auth login` and select **Cerebras**.
...
2. Run `abov3 auth login` and select **DeepSeek**.
...
2. Run `abov3 auth login` and select **Fireworks AI**.
...
To use your GitHub Copilot subscription with abov3:
...
1. Run `abov3 auth login` and select GitHub Copilot.
...
2. Run `abov3 auth login` and select Groq.
...
```json title="abov3.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://abov3.ai/config.json",
  "provider": {
    "lmstudio": {
...
2. Run `abov3 auth login` and select **Moonshot AI**.
...
```json title="abov3.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://abov3.ai/config.json",
  "provider": {
    "ollama": {
...
2. Run `abov3 auth login` and select OpenAI.
...
abov3 zen is a list of tested and verified models provided by the abov3 team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>abov3 zen</a>** and click **Create API Key**.

2. Run `abov3 auth login` and select **abov3 zen**.
...
2. Run `abov3 auth login` and select OpenRouter.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
  ...
  You can also add additional models through your abov3 config.
...
5. You can also customize them through your abov3 config. Here's an example of specifying a provider
...
2. Run `abov3 auth login` and select **Together AI**.
...
For a limited time, you can use xAI's Grok Code for free with abov3.

:::tip
Grok Code is available for free for a limited time on abov3.
:::
...
2. Run `abov3 auth login` and select **Z.AI**.
...
To add any **OpenAI-compatible** provider that's not listed in `abov3 auth login`:

:::tip
You can use any OpenAI-compatible provider with abov3. Most modern AI providers offer OpenAI-compatible APIs.
:::

1. Run `abov3 auth login` and scroll down to **Other**.
...
3. Enter your API key for the provider.
  ...
  $ abov3 auth login
   ...
   ┌  Add credential
   │
  ▲  This only stores a credential for myprovider - you will need configure it in abov3.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `abov3.json` file in your project directory:

   ```json title="abov3.json" ""myprovider"" {5-15}
   {
     "$schema": "https://abov3.ai/config.json",
     "provider": {
       "myprovider": {
