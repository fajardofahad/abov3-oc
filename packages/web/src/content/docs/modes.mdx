...
:::caution
Modes are now configured through the `agent` option in the abov3 config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in abov3 allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the abov3 config.
...
ABOV3 comes with two built-in modes.
...
Configure modes in your `abov3.json` config file:
...
- Global: `~/.config/abov3/mode/`
- Project: `.abov3/mode/`

```markdown title="~/.config/abov3/mode/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global abov3 config and the project specific config.
...
Create mode files in `.abov3/mode/` for project-specific modes or `~/.config/abov3/mode/` for global modes:

```markdown title=".abov3/mode/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/abov3/mode/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
