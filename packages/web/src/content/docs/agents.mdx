...
There are two types of agents in ABOV3; primary agents and subagents.
...
ABOV3 comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
ABOV3 comes with one built-in subagent, **General**. We'll look at this below.

---

## Built-in

ABOV3 comes with two built-in primary agents and one built-in subagent.
...
Configure agents in your `abov3.json` config file:
...
- Global: `~/.config/abov3/agent/`
- Per-project: `.abov3/agent/`

```markdown title="~/.config/abov3/agent/review.md"
---
...
If no temperature is specified, abov3 uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global abov3 config and the project specific config.
...
Configure permissions globally in abov3.json.
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/fajardofahad/abov3-genesis-codeforger).
:::
...
```markdown title="~/.config/abov3/agent/docs-writer.md"
---
...
```markdown title="~/.config/abov3/agent/security-auditor.md"
---
