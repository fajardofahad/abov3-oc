...
Create `.abov3/command/test.md`:

```md title=".abov3/command/test.md"
---
...
You can add custom commands through the abov3 config or by creating markdown files in the `command/` directory.

---

### JSON

Use the `command` option in your abov3 [config](/docs/config):

```json title="abov3.jsonc" {4-12}
{
...
- Global: `~/.config/abov3/command/`
- Per-project: `.abov3/command/`

```markdown title="~/.config/abov3/command/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".abov3/command/component.md"
---
...
```md title=".abov3/command/analyze-coverage.md"
---
...
```md title=".abov3/command/review-changes.md"
---
...
```md title=".abov3/command/review-component.md"
---
...
ABOV3 includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
