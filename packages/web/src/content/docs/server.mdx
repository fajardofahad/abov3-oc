---
title: Server
description: Interact with abov3 server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `abov3 serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an abov3 client can use.
...
```bash
abov3 serve [--port <number>] [--hostname <string>]
```
...
When you run `abov3` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the abov3 server to interact with abov3 programmatically.
:::

This architecture lets abov3 support multiple clients and allows you to interact with abov3 programmatically.

You can run `abov3 serve` to start a standalone server. If you have the
abov3 TUI running, `abov3 serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the abov3 [IDE](/docs/ide) plugins.
...
The abov3 server exposes the following APIs.
...
| `POST`   | `/session/:id/message`                   | Send chat message                  | body matches [`ChatInput`](https://github.com/fajardofahad/abov3-genesis-codeforger/blob/main/packages/opencode/src/session/index.ts#L358), returns <a href={typesUrl}><code>Message</code></a>     |
| `POST`   | `/session/:id/shell`                     | Run a shell command                | body matches [`CommandInput`](https://github.com/fajardofahad/abov3-genesis-codeforger/blob/main/packages/opencode/src/session/index.ts#L1007), returns <a href={typesUrl}><code>Message</code></a> |
