...
Most popular providers are preloaded by default. If you've added the credentials for a provider through `abov3 auth login`, they'll be available when you start abov3.
...
Here are the ones we recommend with abov3:
...
```jsonc title="abov3.jsonc" {7-12,19-24}
{
  "$schema": "https://abov3.ai/config.json",
  "provider": {
    "openai": {
...
When abov3 starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the abov3 config.
