...
With abov3 you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, abov3 uses our own `abov3` theme.
...
abov3 comes with several built-in themes.
...
- Want abov3 to match their terminal's appearance
...
abov3 supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/abov3/themes/*.json` or `$XDG_CONFIG_HOME/abov3/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.abov3/themes/*.json`
4. **Current working directory** - Defined in `./.abov3/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/abov3/themes
vim ~/.config/abov3/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .abov3/themes
vim .abov3/themes/my-theme.json
```
...
{
  "$schema": "https://abov3.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
