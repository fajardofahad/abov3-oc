...
declare global {
  const ABOV3_TUI_PATH: string
}

if (typeof ABOV3_TUI_PATH !== "undefined") {
  await import(ABOV3_TUI_PATH as string, {
    with: { type: "file" },
  })
}
      ...
      const result = await bootstrap(cwd, async () => {
        ...
        const proc = Bun.spawn({
          ...
          env: {
            ...process.env,
            CGO_ENABLED: "0",
            ABOV3_SERVER: server.url.toString(),
            ABOV3_PROJECT: JSON.stringify(Instance.project),
          },
          onExit: () => {
            server.stop()
          },
        ...
        ;(async () => {
          ...
          if (config.autoupdate === false || Flag.ABOV3_DISABLE_AUTOUPDATE) return
...
/**
 * Get the correct command to run ABOV3 CLI
 * In development: ["bun", "run", "packages/opencode/src/index.ts"]
 * In production: ["/path/to/opencode"]
 */
function getOpencodeCommand(): string[] {
  // Check if ABOV3_BIN_PATH is set (used by shell wrapper scripts)
  if (process.env["ABOV3_BIN_PATH"]) {
    return [process.env["ABOV3_BIN_PATH"]]
  }
