...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    for (const file of ["abov3.jsonc", "abov3.json"]) {
    ...
    if (Flag.ABOV3_CONFIG) {
      result = mergeDeep(result, await loadFile(Flag.ABOV3_CONFIG))
      log.debug("loaded custom config", { path: Flag.ABOV3_CONFIG })
    }

    if (Flag.ABOV3_CONFIG_CONTENT) {
      result = mergeDeep(result, JSON.parse(Flag.ABOV3_CONFIG_CONTENT))
      log.debug("loaded custom config from ABOV3_CONFIG_CONTENT")
    }
    ...
    const markdownAgents = [
      ...
      ...(await Filesystem.globUp(".abov3/agent/**/*.md", Instance.directory, Instance.worktree)),
    ]
    for (const item of markdownAgents) {
      ...
      const agentFolderPath = item.includes("/.abov3/agent/")
        ? item.split("/.abov3/agent/")[1]
        : item.includes("/agent/")
          ? item.split("/agent/")[1]
          : agentName + ".md"
    ...
    const markdownModes = [
      ...
      ...(await Filesystem.globUp(".abov3/mode/*.md", Instance.directory, Instance.worktree)),
    ]
    ...
    const markdownCommands = [
      ...
      ...(await Filesystem.globUp(".abov3/command/*.md", Instance.directory, Instance.worktree)),
    ]
    ...
    result.plugin.push(
      ...[
        ...
        ...(await Filesystem.globUp(".abov3/plugin/*.{ts,js}", Instance.directory, Instance.worktree)),
      ].map((x) => "file://" + x),
    )

    if (Flag.ABOV3_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ABOV3_PERMISSION))
    }
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://abov3.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://abov3.ai/docs/agent"),
      provider: z
        .record(
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "abov3.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "abov3.jsonc"))),
    )
    ...
    await import(path.join(Global.Path.config, "config"), {
      ...
      .then(async (mod) => {
        ...
        result["$schema"] = "https://abov3.ai/config.json"
  ...
  async function load(text: string, configFilepath: string) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema) {
        parsed.data.$schema = "https://abov3.ai/config.json"
